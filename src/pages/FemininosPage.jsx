...
export function FemininosPage() {
  ...
  const feminineProducts = useMemo(() => {
    let baseProducts = allProducts.filter(product => 
      product.category === "Feminino" && product.type === "Importado"
    );
  ...
  return (
    <main className="container mx-auto px-4 py-6 flex-grow">
      <div className="text-sm text-gray-600 mb-3">
        <span>Início</span> <span className="mx-2">›</span> <span className="text-amber-800">FEMININOS</span>
      </div>

      <div className="flex items-center justify-between mb-6">
        <h1 className="text-4xl font-bold text-gray-800">PERFUMES FEMININOS</h1>
        <div className="text-sm text-gray-600">
          {feminineProducts.length} produto(s) encontrado(s)
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-8">
        <Button 
          ...
          onClick={() => openFiltersWithContext(feminineProducts)}
        >
